...
class AutoEncoderTopK(Dictionary, nn.Module):
    ...
    def __init__(self, activation_dim: int, dict_size: int, k: int):
        ...
        self.register_buffer("norm_factor", t.tensor(1.0))
        self.register_buffer("shift_factor", t.zeros(activation_dim, dtype=t.float32))
    ...
    def denormalize(self, x: t.Tensor):
    ...
    def set_normalization(self, norm_factor: float, shift: t.Tensor):
...
class TopKTrainer(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        f, top_acts_BK, top_indices_BK, post_relu_acts_BF = self.ae.encode(
            x_norm, return_topk=True, use_threshold=False
        )
    ...
    def update(self, step, x):
        ...
        if step == 0:
            median = geometric_median(self.ae.normalize(x))
