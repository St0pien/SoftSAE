...
def trainSAE(
    ...
    if use_wandb:
        assert len(runs) == len(
            trainer
        ), f"If using wandb you should provide a wandb run foreach trainer: runs - {len(runs)} trainers - {len(trainers)}"
    ...
    if save_dir is not None:
        ...
        for trainer, dir in zip(trainers, save_dirs):
            ...
            try:
                ...
                pass
            with open(
                os.path.join(dir, f"config_{trainer.config["wandb_name"]}.json"), "w"
            ) as f:
    ...
    for save_dir, trainer in zip(save_dirs, trainers):
        ...
        if save_dir is not None:
            ...
            path = os.path.join(save_dir, f"{trainer.config["wandb_name"]}.pt")
            t.save(final, path)
            paths.append(path)
